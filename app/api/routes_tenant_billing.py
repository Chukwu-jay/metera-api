...
async def tenant_billing_overview(request: Request, tenant_id: str | None = None) -> TenantBillingOverviewResponse:
    ...
    latest_report = None
    if current_period_row is not None:
        latest_report = BillingReportSummary(**(await repository.generate_billing_report(billing_period_id=current_period_row["id"], export_format="json")))
...
@router.get("/billing/periods/{billing_period_id}/report", response_model=BillingReportSummary)
async def tenant_billing_report(request: Request, billing_period_id: str, tenant_id: str | None = None, format: str = "json") -> BillingReportSummary:
    scope = await _resolve_scope(request, tenant_id)
    require_tenant_capability(scope, "billing:read")
    repository = _require_billing_repository(request)
    periods = await repository.list_billing_periods(tenant_id=scope.tenant_id)
